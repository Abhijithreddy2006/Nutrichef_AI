...
// Dev server middleware for local AI generation
    ...
    configureServer(server: any) {
      server.middlewares.use(async (req: IncomingMessage, res: ServerResponse, next: () => void) => {
        ...
        try {
          ...
          const { ingredients } = JSON.parse(rawBody) as { ingredients: string }
          ...
          const rawGroq = (env.VITE_GROQ_API_KEY || env.GROQ_API_KEY || process.env.VITE_GROQ_API_KEY || process.env.GROQ_API_KEY || '').trim()
          const rawOpenRouter = (env.OPENROUTER_API_KEY || env.VITE_OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY || process.env.VITE_OPENROUTER_API_KEY || '').trim()
          const rawGemini = (env.GEMINI_API_KEY || env.VITE_GEMINI_API_KEY || process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY || '').trim()
          const rawOpenAi = (env.VITE_OPENAI_API_KEY || env.OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY || process.env.OPENAI_API_KEY || '').trim()
